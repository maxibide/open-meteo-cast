...
def test_init_not_new(mock_load_from_db, mock_get_last_run_timestamp, mock_retrieve_metadata, mock_config, mock_metadata):
...
def test_init_new_run(mock_save_to_db, mock_calculate_statistics, mock_retrieve_model_variable, mock_get_last_run_timestamp, mock_retrieve_metadata, mock_datetime, mock_config, mock_metadata):
    mock_retrieve_metadata.return_value = mock_metadata
...
def test_load_from_db(mock_check_if_new, mock_load_statistics, mock_load_raw_data, mock_get_last_run_timestamp, mock_retrieve_metadata, mock_config, mock_metadata):
    model = WeatherModel("gfs025", mock_config)
...
@patch('src.open_meteo_cast.weather_model.retrieve_model_variable')
@patch('src.open_meteo_cast.weather_model.WeatherModel.calculate_statistics')
def test_save_to_db(mock_calculate_statistics, mock_retrieve_model_variable, mock_check_if_new, mock_save_statistics, mock_save_raw_data, mock_save_forecast_run, mock_get_db_connection, mock_retrieve_metadata, mock_datetime, mock_config, mock_metadata):
    mock_retrieve_metadata.return_value = mock_metadata
    mock_datetime.now.return_value = datetime(2023, 3, 15, 12, 15, 0) # 15 minutes after init
